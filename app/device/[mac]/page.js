...
function DevicePortsPage({ params }) {
  ...
  const [ports, setPorts] = useState([]);
  const [newPortName, setNewPortName] = useState('');
  const [newPortPin, setNewPortPin] = useState(1); // Default pin 1
  ...
  const handleAddPort = async (e) => {
    ...
    let backendPin = Number(newPortPin);
    if (isNaN(backendPin) || backendPin < 1) backendPin = 1;
    ...
    try {
      ...
      if (result && result.success) {
        ...
        const errorMsg = (result && result.error) || 'Failed to add port';
        alert(`Error: ${errorMsg}`);
      }
    } catch (error) {
      const errorMsg = error?.message || 'An unexpected error occurred';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <style>{`
        .header { position: fixed; top:0; left:0; right:0; background: rgba(255,255,255,0.95); backdrop-filter: blur(8px); border-bottom: 1px solid rgba(229,231,235,0.6); z-index: 50; padding: 0.6rem 1rem; }
        .header-container { display:flex; align-items:center; justify-content:space-between; gap:1rem; max-width:1200px; margin:0 auto; }
        .logo img { height:36px; display:block; }
        .gradient-btn { display: inline-flex; align-items:center; gap:0.5rem; padding: 0.45rem 0.85rem; border-radius: 0.6rem; background: linear-gradient(90deg,#ef4444,#ef4444); color: white; font-weight:700; border:none; cursor:pointer; box-shadow: 0 6px 18px rgba(239,68,68,0.12); transition: background 0.3s; }
        .gradient-btn:hover { background: linear-gradient(90deg,#dc2626,#b91c1c); }
        @media (max-width: 420px) { .gradient-btn { padding: 0.35rem 0.6rem; font-size: 0.9rem; } }
      ...
      <header className="header">
        <div className="header-container flex justify-between items-center">
          <Link href="/" className="logo flex items-center" aria-label="Home">
            <img src="/assets/images/Nlogo.png" alt="SmartRemote Logo" className="h-9" />
          </Link>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleLogout} className="gradient-btn" title="Logout">
              Logout
            </button>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">{deviceName}</h2>
          {!nameParam && <div className="text-sm text-gray-500 mb-6">{mac}</div>}

          {/* Add Port Form */}
          <div className="bg-white shadow overflow-hidden sm:rounded-lg mb-8">
            <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">Add New Port</h3>
              <p className="mt-1 text-sm text-gray-500">Add a new switch/port to this device</p>
            </div>
            <div className="px-4 py-5 sm:p-6">
              <form onSubmit={handleAddPort} className="space-y-6">
                <div className="grid grid-cols-6 gap-6">
                  <div className="col-span-6 sm:col-span-4">
                    <label htmlFor="port-name" className="block text-sm font-medium text-gray-700">Port Name</label>
                    <input
                      type="text"
                      name="port-name"
                      id="port-name"
                  ...
                  <div className="col-span-6 sm:col-span-2">
                    <label htmlFor="port-pin" className="block text-sm font-medium text-gray-700">Pin Number</label>
                    <select
                      id="port-pin"
                      value={newPortPin}
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">Ports / Switches</h3>
            </div>
            <div className="bg-white overflow-hidden">
              {ports.length === 0 ? (
                <div className="text-center py-12">
                  <p className="mt-1 text-sm text-gray-500">No ports yet. Add one above.</p>
                </div>
              ) : (
                <ul className="divide-y divide-gray-200">
                  {ports.map((port) => (
                    <li key={port.pin}>
                      ...
                      >
                        <div className="px-4 py-4 sm:px-6 flex justify-between items-center">
                          <p className="text-sm font-medium text-blue-600 truncate">{port.name}</p>
                          <p className="px-2 inline-flex text-xs font-semibold rounded-full bg-green-100 text-green-800">
                            Pin {port.pin}
                          </p>
                        </div>
                      </Link>
