...
// Helper to format local date/time as ISO-preserving local values (keeps UI time)
const toUTCISOString = (date) => {
  if (!date) return null;
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, '0');
...
function IndividualSwitchControlPage({ params }) {
  ...
  const deviceName = nameParam || `Device ${mac}`;
  const portName = searchParams.get('portName') || `Switch ${pin}`;
  const router = useRouter();

  const {
    getPinState: fetchPinStatus,
    togglePinState: togglePinStatus,
    fetchSchedules,
    createSchedule,
    deleteSchedule,
    loading,
  ...
  const getBackendPin = (pinVal) => {
    const pinNum = parseInt(pinVal, 10);
    return pinNum > 0 ? pinNum - 1 : 0;
  };
  ...
  const handleToggleSwitch = async () => {
    ...
    setIsOn(prev => !prev);
    const result = await togglePinStatus(mac, backendPin, 'toggle');
    if (!result.success) setIsOn(prev => !prev);
    else await fetchCurrentPinStatus();
  };

  const handleCreateSchedule = async () => {
    if (!formSchedule.name.trim() && !formSchedule.onTime && !formSchedule.offTime) {
      alert("Provide a schedule name and at least one time.");
      return;
    }
    const backendPin = getBackendPin(pin);
    const scheduleData = {
    ...
    const result = await createSchedule(mac, backendPin, scheduleData);
    if (result.success) {
      setFormSchedule({ name: '', onTime: null, offTime: null });
      fetchPinSchedules();
    }
  };
  ...
  const handleDeleteSchedule = async (scheduleId) => {
    ...
    const backendPin = getBackendPin(pin);
    const result = await deleteSchedule(mac, backendPin, scheduleId);
    if (result.success) fetchPinSchedules();
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 p-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link href="/" className="flex items-center gap-2">
            <img src="/assets/images/Nlogo.png" alt="Logo" className="h-9" />
            {/* <span className="font-bold text-lg text-gray-900">SmartRemote</span> */}
          </Link>
          <button
            onClick={handleLogout}
            className="flex items-center gap-1 px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-md shadow transition"
          >
            Logout
          </button>
        </div>
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <button
          onClick={() => router.back()}
          className="flex items-center text-blue-600 hover:text-blue-800 font-medium mb-4 group transition-colors duration-200"
        >
          <svg
          ...
          <span className="relative group-hover:underline">{`Back to ${deviceName}`}</span>
        </button>

        {/* Switch Card */}
        <div className="bg-white shadow-lg rounded-lg p-6 flex flex-col sm:flex-row sm:justify-between sm:items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">{portName}</h2>
            <p className="text-gray-500 mt-1">Device: {deviceName}{!nameParam && ` (MAC: ${mac})`}</p>
          </div>
          <div className="flex items-center mt-4 sm:mt-0 gap-4">
            <span className={`px-3 py-1 rounded-full font-semibold ${isOn ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
              {isOn ? 'ON' : 'OFF'}
            </span>
            <button
              onClick={handleToggleSwitch}
              disabled={loading}
              className={`px-4 py-2 rounded-md shadow text-white font-medium transition ${loading ? 'bg-gray-400 cursor-not-allowed' : isOn ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700'}`}
            >
        ...
        <div className="bg-white shadow-lg rounded-lg p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <input
              ...
              className="border rounded-md p-2 shadow-sm focus:ring-2 focus:ring-blue-500 w-full"
            />
            <DatePicker
              selected={formSchedule.onTime}
              onChange={(date) => setFormSchedule({...formSchedule, onTime: date})}
              showTimeSelect
              timeIntervals={10} // 10-minute slots
              timeCaption="Time"
              dateFormat="MMMM d, yyyy h:mm aa"
              placeholderText="Turn ON"
              className="border rounded-md p-2 shadow-sm focus:ring-2 focus:ring-green-500 w-full"
            />
            <DatePicker
              selected={formSchedule.offTime}
              onChange={(date) => setFormSchedule({...formSchedule, offTime: date})}
              ...
              className="border rounded-md p-2 shadow-sm focus:ring-2 focus:ring-red-500 w-full"
            />
            <button
              onClick={handleCreateSchedule}
              disabled={loading}
              className="sm:col-span-3 mt-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md py-2 shadow transition"
            >
              {loading ? 'Adding...' : 'Add Schedule'}
            </button>
          </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {schedules.map(schedule => (
                    <tr key={schedule.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4">{new Date(schedule.time).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 text-xs rounded-full font-semibold ${schedule.action === 'Turn On' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                          {schedule.action}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => handleDeleteSchedule(schedule.id)} className="text-red-600 hover:text-red-900 font-medium">Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
